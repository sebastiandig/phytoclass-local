...
# corrected for minimise_elements_2
Steepest_Descent <- function(Fmat, place, S, cm, num.loops) {
  loop <- 1
  F.new <- NNLS_MF(Fmat, S, cm)
  F.initial <- F.new
  for (i in 1:num.loops) { # should always be small. It would be nice to allow the
    ...
    while (F.new[[2]] > F.initial[[2]]) {
      ...
      #
      # # F.new <- Minimise_elements(F.initial[[1]], place, S, cm)
      # F.new <- Minimise_elements_2(F.initial[[1]], place, S, cm, .num = 3, .fac_rr = 3)
      #
      # if (loop_2 > 5) {
      #   # F.new <- Minimise_elements1(F.initial[[1]], place, S, cm)
      #   F.new  <- Minimise_elements_2(F.initial[[1]], place, S, cm, .num = 1, .fac_rr = 1)
      #
      # } # If it doesn't work the first time, it randomises at a lower rate
      # if (loop_2 > 10) {
      #   # F.new <- Minimise_elements2(F.initial[[1]], place, S, cm)
      #   F.new <- Minimise_elements_2(F.initial[[1]], place, S, cm, .num = 2, .fac_rr = 2)
      #
      # } # and again,
